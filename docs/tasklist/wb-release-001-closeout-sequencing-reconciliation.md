...
specification_revision: define-r1-2026-08-24
status: in_progress
---
...
- [ ] TASK-007 [type=requirement] [req=REQ-002] [ac=AC-003,AC-004] [paths=scripts/validate-release-state.py] Validate one declared local-only candidate, its terminal projection, predecessor binding, and non-promotion boundary.
- [ ] TASK-008 [type=requirement] [req=REQ-003] [ac=AC-005] [paths=scripts/validate-release-state.py] Add deterministic candidate-manifest/evidence-only-persistence verification with no hidden normative delta.
- [ ] TASK-009 [type=requirement] [req=REQ-004] [ac=AC-006] [paths=governance/release-state.md,.agent/workflows/sdd-protocol.md,FILE_REGISTRY.yml] Synchronize the normative contract, operational procedure, and machine-readable acceptance sequence.
- [ ] TASK-010 [type=requirement] [req=REQ-005] [ac=AC-007] [paths=scripts/test-release-state-contracts.py] Add positive and adversarial fixtures for ordinary and candidate modes.
- [ ] TASK-011 [type=requirement] [req=REQ-006] [ac=AC-008] [paths=governance/release-state.md,.agent/workflows/sdd-protocol.md,FILE_REGISTRY.yml,scripts/validate-release-state.py,scripts/test-release-state-contracts.py] Preserve bounded scope and prove the frozen source manifest excludes historical Work Blocks and unrelated implementation.
...
permit a pushed/PR/CI candidate, require changes to historical completed Work
Blocks, require a new dependency, or require an unplanned template-wide change.
