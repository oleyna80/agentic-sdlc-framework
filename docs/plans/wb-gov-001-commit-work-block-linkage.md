...
branch: agent/wb-gov-001-final-clean
base_revision: 1f508bf6eefc577951a2102685a41a94e4bdd949
...
The valid implementation lineage is H3
`1f508bf6eefc577951a2102685a41a94e4bdd949` → H7
`308ffc58bba73820987f9e1b7a1e9774514c02cd`. Historical H4/H5/H6 and the
old promoted P1 are excluded. C3 records the clean candidate before the four
r3 assurance artifacts are persisted.
...
1. The hook uses `git rev-parse`, isolated `git interpret-trailers`, and Python
   JSON parsing available in the generated framework contract. Git system and
   global configuration are excluded from trailer parsing, so only the literal
   canonical `Work-Block` key is accepted; repository-local trailer aliases and
   separators cannot remap the contract. It resolves the repository root and
   never depends on Codex, Claude, OpenCode, or branch names.
