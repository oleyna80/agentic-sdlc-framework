...
- Replace runtime-named framework profiles with governance profiles plus runtime
  adapter selection.
- Copy the governance core into generated projects under `governance/`.
...
- [x] Generated projects contain `governance/` copied from one framework source.
- [x] Core roles are Owner, Orchestrator, Architect, Critic, Coder, Reviewer,
      and Verifier.
- [x] Runtime, model class, specialization, isolation, and tool availability are
      recorded separately from role authority.
- [x] Specification and acceptance criteria outrank implementation plans and
      tasklists.
- [x] Review, Verification, and Drift Audit have distinct inputs, outputs, and
      verdicts.
- [x] Governance profile and runtime adapter are selected independently.
- [x] A generic sequential agent can follow the contract without native
      subagents or provider-specific commands.
- [x] Existing Claude Code and Codex compatibility layers remain available.
- [x] Contract tests detect reintroduction of provider-specific core authority or
...
- GitHub compare confirms the branch remains based on current `main` and is not behind.
- Contract-test source was inspected against the changed portable contracts.
- Local shell execution remains pending because the connector environment does not provide a repository checkout.
- PR remains draft until the shell checks run in a normal checkout or CI.
